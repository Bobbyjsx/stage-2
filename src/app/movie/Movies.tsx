...
interface _MovieProps {
	seeFull: boolean;
}

export default function Movies({ seeFull = true }: _MovieProps) {
	let path;

	seeFull ? (path = "/movie/popular") : (path = "/movie/top_rated");
		...
		return (
			<main className="flex flex-col justify-center items-center w-full mx-auto">
				<div className="grid grid-cols-1 md:grid-cols-4 place-items-center gap-20">
					{movieList?.results.map((tile: Movie) => {
						return (
							<div
